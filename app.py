import streamlit as st
import pandas as pd
from datetime import datetime

# إعدادات الصفحة الأساسية
st.set_page_config(page_title="معمل أسلوب الأناقة", page_icon="✂️", layout="wide")

# العنوان الرئيسي
st.markdown("<h2 style='text-align: center; color: #1e293b;'>معمل أسلوب الأناقة - نظام الإدارة الشامل</h2>", unsafe_allow_html=True)
st.markdown("<p style='text-align: center; color: #64748b;'>لوحة التحكم السحابية لإدارة إنتاج الخياطين ومعمل الزرار</p>", unsafe_allow_html=True)
st.divider()

# تهيئة قاعدة البيانات المؤقتة
if 'data' not in st.session_state:
    st.session_state.data = pd.DataFrame(columns=['التاريخ', 'اسم الخياط', 'عدد القطع'])

# الشريط الجانبي للإدخال
st.sidebar.header("📝 تسجيل إنتاج يومي")
with st.sidebar.form("entry_form"):
    entry_date = st.date_input("التاريخ", datetime.today())
    tailor_name = st.selectbox("اختار العامل:", [
        "عبد الله", "إدريس", "رام", "سبدول", 
        "نارش", "سجاد", "إرشاد", "بدرول"
    ])
    pieces_count = st.number_input("عدد القطع", min_value=1, value=1, step=1)
    
    submit_button = st.form_submit_button(label="حفظ السجل")

    if submit_button:
        new_row = pd.DataFrame({
            'التاريخ': [str(entry_date)],
            'اسم الخياط': [tailor_name],
            'عدد القطع': [int(pieces_count)]
        })
        st.session_state.data = pd.concat([st.session_state.data, new_row], ignore_index=True)
        st.sidebar.success("تم الحفظ بنجاح!")

# العرض الرئيسي للبيانات
if not st.session_state.data.empty:
    total_pieces = st.session_state.data['عدد القطع'].sum()
    
    col1, col2, col3 = st.columns(3)
    col1.metric("إجمالي القطع المسجلة", total_pieces)
    col2.metric("حساب معمل الزرار", f"{total_pieces * 5} ر.ي")
    col3.metric("مستحقات الخياطين", f"{total_pieces * 15} ر.ي")
    
    st.divider()
    st.subheader("📊 سجلات الإنتاج اليومية")
    st.dataframe(st.session_state.data, use_container_width=True)
    
    if st.button("مسح كافة البيانات"):
        st.session_state.data = pd.DataFrame(columns=['التاريخ', 'اسم الخياط', 'عدد القطع'])
        st.rerun()
else:
    st.info("لا توجد سجلات مسجلة حتى الآن. استخدم القائمة الجانبية لإدخال البيانات.")
